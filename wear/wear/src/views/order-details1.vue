...
export default {
  ...
  computed: {
    ...mapState(["index",'total',"sort", "order_detail_msg","loginUser",'getDelAid']),
  },
//   created() {
...
//   },
  mounted() {
    ...
    this.axios.get(`/shop/get_aid?aid=${this.getDelAid}`).then(res=>{
    ...
    tanchu() {
      this.$dialog
        ...
        .then(() => {
          ...
          // console.log(this.number)
          this.axios
            .get(`/shop/det-order?aid=${this.getDelAid}`)
            .then((res) => {
              // console.log(res.data)
              // console.log(111)
                this.$notify({
