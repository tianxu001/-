<template>
  <div class="content">
    <div class="shoplist">
      <div v-for="(item,index) in shopping_shoesort" :key="index">
        <router-link to="/">
          <img v-lazy="item.simg" />
          <p>{{item.smessage}}</p>
        </router-link>
      </div>
    </div>
    ...
    <router-link to="/">
      <div class="advise">
        <img src="../assets/img/index/shop/shoes.png" alt />
      </div>
    </router-link>
    <!-- 秒杀结束 -->
...
export default {
  ...
  mounted() {
    this.axios.get("/shop/shopping_shoesort").then((res) => {
      let data = res.data.results;
      data.forEach((item) => {
        item.simg = require("../assets/img/commodity/" + item.simg);
        this.shopping_shoesort.push(item);
      });
    });

    this.axios.get("/shop/sort_id?fid=2").then((res) => {
      let data = res.data;
      data.forEach((item) => {
